...
@testset "SmoothedDisplacement" begin
    @test SmoothedDisplacement <: ImageTransformation
    @test multiplier(SmoothedDisplacement(10,10)) == 1
    op = SmoothedDisplacement(4,5)
    @test op == SmoothedDisplacement(4., 5.)
    show(op); println()
    showcompact(op); println()
    @test op.gridwidth == 4
    @test op.gridheight == 5
    ...
    op = SmoothedDisplacement(10,12, scale = .4, sigma = 4, iterations = 9, normalize = false)
    @test op.gridwidth == 10
    @test op.gridheight == 12
    @test op.scale == .4
    @test op.sigma == 4.
    ...
    @test op.static_border == true
    @test op.normalize == false
    op = SmoothedDisplacement(10,12, scale = 2, sigma = 3, iterations = 2, static_border = false)
    @test op.gridwidth == 10
    @test op.gridheight == 12
    @test op.scale == 2.
    @test op.sigma == 3.
    ...
    @test_throws ArgumentError SmoothedDisplacement(2,5)
    @test_throws ArgumentError SmoothedDisplacement(4,2)
    @test_throws ArgumentError SmoothedDisplacement(4,5, sigma = 0.)
    @test_throws ArgumentError SmoothedDisplacement(4,5, sigma = -1.)
    @test_throws ArgumentError SmoothedDisplacement(4,5, iterations = 0)

    srand(123)
    @imagetest "SmoothedRandomDisplacement" transform(SmoothedDisplacement(4,5), testimg)
end
